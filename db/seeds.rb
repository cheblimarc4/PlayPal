...
names = ["Margarida", "João", "David", "Ines", "Rita", "Henrique", "Marta Rosa", "Mariana", "Francisco P1", "Francisco P2", "Gonçalo","Marc", "Teresa", "Guillaume", "Lucelia", "Zainan", "Afonso"]
...
UserMatch.where(user: User.find_by(first_name: "David")).each do |usermatch|
  ...
  players_in_match = usermatch.match.need + usermatch.match.UserMatches.where(status: "accepted").count
  puts "#{players_in_match} players in match"
...
matches.each do |match|
  ...
  match.update(team_a_score: rand(0..10), team_b_score: rand(0..10), winning_team: rand(1..2))
end
